...
def analyze_shallow(combat:Combat, settings):
    ...
    combat_duration = (combat.log_data[-1].timestamp - combat.date_time).total_seconds()
    
    for player in player_dict.values():
        player.combat_time = combat_duration
        successful_attacks = player.total_attacks - player.misses
        try:
            player.debuff = player.resistance_sum / player.hull_attacks
        except ZeroDivisionError:
            player.debuff = 0.0
        try:
            player.DPS = player.total_damage / combat_duration
        except ZeroDivisionError:
            player.DPS = 0.0
